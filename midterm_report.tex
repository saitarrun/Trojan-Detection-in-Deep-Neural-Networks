...
\begin{figure}[tbp]
  ...
  \begin{tikzpicture}[
    ...
    % --- Left branch: clean ---
    \node[block] (cd)  {Clean CIFAR-10};
    \node[block, below=of cd] (cp) {ResNet-18\\Training};
    \node[block, below=of cp] (cm) {\texttt{clean\_model.pth}};
    \draw[line] (cd) -- (cp);
    \draw[line] (cp) -- (cm);

    % --- Right branch: poisoned ---
    \node[block, right=of cd] (pd)  {\texttt{BadNetsDataset}\\(10\% poison)};
    \node[block, below=of pd] (pp) {ResNet-18\\Training};
    \node[block, below=of pp] (pm) {\texttt{poisoned\_model.pth}};
    \draw[line] (pd) -- (pp);
    \draw[line] (pp) -- (pm);

    % --- Inference ---
    \node[block, below=1.4cm of cm]  (ci) {Clean Input $x$};
    \node[block, below=1.4cm of pm]  (ti) {Triggered Input $x\oplus\tau$};

    \node[block, below=0.9cm of ci] (co) {Correct Output};
    \node[block, below=0.9cm of ti] (to) {Target Class $y_t$};
    ...
    \draw[line] (pm) -- (to);
  \end{tikzpicture}
  \caption{Threat model: clean vs.\ Trojaned ResNet-18 produced by
    \texttt{train.py}.  Both models achieve comparable CDA yet
    \texttt{poisoned\_model.pth} misclassifies any triggered input as $y_t$.}
...
\begin{table}[tbp]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    \texttt{app.py}              & Streamlit dashboard (UI, audit, visualisation) \\
...
\begin{table}[tbp]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    \texttt{clean\_label}         & Checkerboard applied only to samples of the target class; no relabelling \\
    \texttt{dynamic}              & Random position and random pattern regenerated per sample \\
    \texttt{instagram\_filter}    & Global sepia colour-cast via $3\times3$ colour-matrix multiplication \\
...
\begin{table}[tbp]
  ...
  \begin{tabular}{@{}llc@{}}
    \toprule
    \textbf{Input Signal}      & \textbf{Normalisation}                              & \textbf{Weight} \\
    \midrule
    Neural Cleanse (NC)        & $\min\bigl((\text{max\_idx}-2.0)/2.0,\;1\bigr)$ if max\_idx$\geq2.0$ & 0.20 \\
    STRIP FR/FA ratios         & $1.0 - (\text{avg\_error}\times 2.0)$              & 0.25 \\
    Clustering silhouette      & $(\text{score}-0.05)/0.20$; zero if $<0.05$        & 0.15 \\
    Weight Analysis (LWA)      & $(\text{max\_idx}-2.5)/2.5$; zero if $<2.5$        & 0.15 \\
    Natural Trojan drift (NTP) & $\min(\text{sensitivity}\times1.5,\;1.0)$          & 0.25 \\
    \bottomrule
  \end{tabular}
\end{table}
...
Base64-encoded string and rendered in the Streamlit dashboard under
\emph{Mechanistic Interpretability}.

% -------------------------------------------------------
\subsection{System Architecture and Dashboard}

Figure~\ref{fig:architecture} shows the full layered architecture.  The
Streamlit dashboard offers two audit modes:
...
\begin{itemize}
  ...
  \item \textbf{Manual forensic analysis (local mode).}  Each defence module
        runs directly in the Streamlit process and results are shown section
        by section.
\end{itemize}
...
\begin{figure}[tbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[block] (ui)   {Streamlit UI (\texttt{app.py}): model upload / trigger config / audit trigger / heatmap viewer};
  ...
  \caption{Layered system architecture of the Trojan Detection project.
    Each layer communicates strictly downward.}
...
\begin{table}[tbp]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    Python                   & 3.10+ \\
    Deep learning            & PyTorch (MPS backend on Apple Silicon; CUDA on Linux/GPU) \\
